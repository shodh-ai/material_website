...
The homepage (`/`) and the assets under `/shodh-new/` are public. Every other
...
```text
VIEWER_ACCESS_RULES={"replace-with-a-long-random-code":["/founders-associate","/Founders_Associate.png"],"replace-with-a-different-long-code":["/investor/data-room","/pdf","/pre-prints","/Shodh_Industrial_Validation_Portfolio_Final_Polished.pdf"]}
```
...
For a page with files from `public/`, list those file paths or directories in
the same rule. For example, the Founders Associate page needs both
`/founders-associate` and `/Founders_Associate.png`.
