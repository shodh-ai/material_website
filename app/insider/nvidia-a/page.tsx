...
function EngineeringBottlenecksInfographic() {
  const bottlenecks = [
    {
      title: "Data Generation Engine (Triton / XLA)",
      bottleneck: "<strong>XLA HLO graphs OOM</strong> (>64GiB/GPU overhead) at <strong>1B voxels</strong>. Custom <strong>Triton kernel</strong> hits strict <strong>SRAM register-spilling</strong> on the full 27-moment non-linear Cumulant recursion.",
      target: "Scale <strong>1B-voxel throughput</strong> from 6.3 GLUPS → <strong>20–50 GLUPS</strong>.",
      ask: "<strong>Warp-Level Kernel Fission</strong> & <strong>Algebraic Unrolling</strong>. We need internal Triton team support to route <strong>sparse PDEs</strong> directly to <strong>CUDA cores</strong>, bypassing <strong>Tensor Core 32x32 tile-padding</strong> overhead and SRAM limits.",
    },
    {
      title: "Distributed Training & Networking (NCCL)",
      bottleneck: "<strong>NCCL stalls</strong> on <strong>3D Spatial Tensor Parallelism</strong>. Standard 1D-optimized LLM parallelism chokes on dense, high-frequency volumetric halo exchanges.",
      target: "<strong>&lt;6 ms latency</strong> for 13+ GiB <strong>3D boundary swaps</strong> (ensuring &lt;1% step overhead).",
      ask: "Co-develop <strong>asynchronous, topology-aware 3D halo-exchange primitives</strong> in NCCL to perfectly overlap volumetric spatial communication with compute.",
    },
    {
      title: "Ecosystem Deployment (Omniverse / Modulus)",
      bottleneck: "Streaming massive <strong>4D extreme multiphysics tensors</strong> (1000:1 Phase-Fields, Rheology) into digital twins breaks standard data-connector I/O limits.",
      target: "<strong>Real-time, zero-copy visualization</strong> of <strong>1-Billion voxel fluid states</strong> natively in <strong>Omniverse</strong>.",
      ask: "<strong>PhysicsNeMo / Modulus API integration</strong> to build high-bandwidth <strong>OpenUSD bridges</strong> for live 3D physics latent streaming.",
    },
  ];
  ...
  return (
    <section className="relative mx-auto w-full max-w-[1500px] overflow-hidden rounded-[1.4rem] border border-black/10 bg-[#f6f5ef] p-5 shadow-[0_30px_90px_rgba(8,20,33,0.22)] sm:p-6 md:rounded-[2rem] md:p-8">
      ...
      <div className="relative z-10 mx-auto max-w-[1040px] text-center">
        <p className="text-xs font-bold uppercase tracking-[0.3em] text-[#4b6b00]">Engineering Partnership</p>
        <h2 className="mx-auto max-w-[900px] text-4xl font-bold uppercase leading-none tracking-tight text-[#0b2338] md:text-5xl">SHODH AI target benchmarks</h2>
      </div>

      <div className="relative z-10 mx-auto mt-10 max-w-[1200px] space-y-6">
        {bottlenecks.map((item, index) => (
          <div key={index} className="rounded-2xl border border-[#0b2338]/10 bg-white/72 p-6 shadow-[0_22px_70px_rgba(8,20,33,0.12)] backdrop-blur-xl">
            <h3 className="text-xl font-bold uppercase tracking-tight text-[#0b2338]">{item.title}</h3>
            <div className="mt-4 space-y-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#48cae4]">Bottleneck</p>
                <p className="mt-1 text-sm font-medium leading-relaxed text-[#0b2338]" dangerouslySetInnerHTML={{ __html: item.bottleneck }} />
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#4b6b00]">Target Benchmark</p>
                <p className="mt-1 text-sm font-medium leading-relaxed text-[#0b2338]" dangerouslySetInnerHTML={{ __html: item.target }} />
              </div>
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#0b2338]">Engineering Ask</p>
                <p className="mt-1 text-sm font-medium leading-relaxed text-[#0b2338]" dangerouslySetInnerHTML={{ __html: item.ask }} />
              </div>
            </div>
          </div>
        ))}
        ...
        <section className="pointer-events-auto px-4 pb-24 pt-4 sm:px-6 md:px-10 md:pb-32">
          ...
          >
            <motion.div variants={scaleIn} className="flex items-center justify-center">
              <img src="/no-jax.svg" alt="No JAX" className="h-auto w-full max-w-[1200px] object-contain" />
            </motion.div>
          </motion.div>
        </section>
